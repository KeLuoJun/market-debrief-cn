...
**适用指数**：沪深300, 创业板指, 科创50, 中证500

**`fetch_market_data.py` 输出的 `valuation_data` 结构**：
...
**注意**：此接口数据来源可能不稳定，失败时返回空数组。

---

## v2新增：全市场资金流向（按订单大小拆分）

```python
df = ak.stock_market_fund_flow()
```

**字段**：`日期`, `超大单净流入`, `大单净流入`, `中单净流入`, `小单净流入` 等

**`fetch_market_data.py` 输出的 `fund_flow_detail` 结构**：

```json
{
  "latest": { "日期": "2025-03-10", "超大单净流入": -5000000000, ... },
  "recent_data": [...]
}
```
...
**`fetch_market_data.py` 输出的 `bond_yield` 结构**：
...
**用途**：计算ERP（股债性价比） = 沪深300股息率 - 10年期国债收益率

---

## v2新增：涨停板生态分析

非直接API调用，而是从全市场快照 `stock_zh_a_spot_em()` 中衍生计算：
- 涨停总数（涨跌幅 >= 9.9%）
- 封板率（收盘价 == 最高价 的涨停股比例）
- 20cm涨停统计（涨跌幅 >= 19.9%，创业板/科创板）
- 集合竞价偏离（(开盘-昨收)/昨收 的全市场均值）

**`fetch_market_data.py` 输出的 `limit_up_ecology` 结构**：
